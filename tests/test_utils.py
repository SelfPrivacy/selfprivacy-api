...
def test_hash_password_passes_password_over_stdin(mocker):
    ...
    popen = mocker.patch(
        "selfprivacy_api.utils.unix_password.subprocess.Popen", return_value=process
    )
...
def test_hash_password_rejects_unsupported_control_characters(mocker, character):
    popen = mocker.patch("selfprivacy_api.utils.unix_password.subprocess.Popen")

    with pytest.raises(PasswordHasInvalidChars):
        hash_password(f"pass{character}word")
