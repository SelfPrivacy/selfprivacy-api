...
async def empty_repo(request, empty_redis_repo):
...
async def some_tokens_repo(empty_repo):
    for name in ORIGINAL_DEVICE_NAMES:
        await empty_repo.create_token(name)
    assert len(await empty_repo.get_tokens()) == len(ORIGINAL_DEVICE_NAMES)
    for name in ORIGINAL_DEVICE_NAMES:
        assert await empty_repo.get_token_by_name(name) is not None
    assert await empty_repo.get_new_device_key() is not None
    return empty_repo
...
async def test_get_token_by_token_string(some_tokens_repo):
    repo = some_tokens_repo
    test_token = (await repo.get_tokens())[2]

    assert (
        await repo.get_token_by_token_string(token_string=test_token.token)
    ) == test_token


async def test_get_token_by_non_existent_token_string(some_tokens_repo):
    repo = some_tokens_repo
...
async def test_get_token_by_name(some_tokens_repo):
    ...
    token = await repo.get_token_by_name(token_name="primary_token")
    assert token is not None
    assert token.device_name == "primary_token"
    assert token in await repo.get_tokens()


async def test_get_token_by_non_existent_name(some_tokens_repo):
    repo = some_tokens_repo
...
async def test_is_token_valid(some_tokens_repo):
    repo = some_tokens_repo
    token = (await repo.get_tokens())[0]
    assert await repo.is_token_valid(token.token)
    assert not await repo.is_token_valid("gibberish")


async def test_is_token_name_pair_valid(some_tokens_repo):
    repo = some_tokens_repo
    token = (await repo.get_tokens())[0]
    assert await repo.is_token_name_pair_valid(token.device_name, token.token)
    assert not await repo.is_token_name_pair_valid(token.device_name, "gibberish")
    assert not await repo.is_token_name_pair_valid("gibberish", token.token)


async def test_is_token_name_exists(some_tokens_repo):
    repo = some_tokens_repo
    token = (await repo.get_tokens())[0]
    assert await repo.is_token_name_exists(token.device_name)
    assert not await repo.is_token_name_exists("gibberish")


async def test_get_tokens(some_tokens_repo):
    repo = some_tokens_repo
    tokenstrings = []
    # we cannot insert tokens directly via api, so we check meta-properties instead
    for token in await repo.get_tokens():
...
async def test_create_token(empty_repo, mock_token_generate):
    repo = empty_repo

    assert await repo.create_token(device_name="IamNewDevice") == Token(
        token="ZuLNKtnxDeq6w2dpOJhbB3iat_sJLPTPl_rN5uc5MvM",
        device_name="IamNewDevice",
        created_at=TOKEN_TEST_DATE,
    )
...
async def test_create_token_existing(some_tokens_repo):
    repo = some_tokens_repo
    old_token = (await repo.get_tokens())[0]

    new_token = await repo.create_token(device_name=old_token.device_name)
    assert new_token.device_name != old_token.device_name

    assert old_token in await repo.get_tokens()
    assert new_token in await repo.get_tokens()


async def test_delete_token(some_tokens_repo):
    repo = some_tokens_repo
    original_tokens = await repo.get_tokens()
    input_token = original_tokens[1]

    await repo.delete_token(input_token)

    tokens_after_delete = await repo.get_tokens()
...
async def test_delete_not_found_token(some_tokens_repo):
    repo = some_tokens_repo
    initial_tokens = await repo.get_tokens()
    input_token = Token(
        token="imbadtoken",
...
async def test_refresh_token(some_tokens_repo):
    repo = some_tokens_repo
    input_token = (await some_tokens_repo.get_tokens())[0]

    output_token = await repo.refresh_token(input_token)
    ...
    assert output_token in await repo.get_tokens()


async def test_refresh_not_found_token(some_tokens_repo, mock_token_generate):
    repo = some_tokens_repo
    input_token = Token(
...
async def test_get_recovery_key_when_empty(empty_repo):
    repo = empty_repo

    assert await repo.get_recovery_key() is None


async def test_create_get_recovery_key(some_tokens_repo, mock_recovery_key_generate):
    repo = some_tokens_repo

    assert await repo.create_recovery_key(uses_left=1, expiration=None) is not None
    assert await repo.get_recovery_key() == RecoveryKey(
        key="889bf49c1d3199d71a2e704718772bd53a422020334db051",
        created_at=TEST_DATE,
        expires_at=None,
        uses_left=1,
...
async def test_use_mnemonic_recovery_key_when_empty(empty_repo):
    repo = empty_repo
...
async def test_use_mnemonic_not_valid_recovery_key(
...
async def test_use_mnemonic_expired_recovery_key(
    some_tokens_repo,
):
    repo = some_tokens_repo
    expiration = ten_minutes_into_past()
    assert (
        await repo.create_recovery_key(uses_left=2, expiration=expiration) is not None
    )
    recovery_key = await repo.get_recovery_key()
...
async def test_use_mnemonic_not_mnemonic_recovery_key(some_tokens_repo):
    repo = some_tokens_repo
    assert await repo.create_recovery_key(uses_left=1, expiration=None) is not None
...
async def test_use_not_mnemonic_recovery_key(some_tokens_repo):
    repo = some_tokens_repo
    assert await repo.create_recovery_key(uses_left=1, expiration=None) is not None
...
async def test_use_not_found_mnemonic_recovery_key(some_tokens_repo):
    repo = some_tokens_repo
    assert await repo.create_recovery_key(uses_left=1, expiration=None) is not None
...
async def test_use_mnemonic_recovery_key(some_tokens_repo, recovery_key_uses_left):
    repo = some_tokens_repo
    assert (
        await repo.create_recovery_key(
            uses_left=recovery_key_uses_left, expiration=None
        )
        is not None
    )
    assert await repo.is_recovery_key_valid()
    recovery_key = await repo.get_recovery_key()
...
async def test_get_new_device_key(some_tokens_repo, mock_new_device_key_generate):
    repo = some_tokens_repo

    assert await repo.get_new_device_key() == NewDeviceKey(
        key="43478d05b35e4781598acd76e33832bb",
        created_at=TEST_DATE,
        expires_at=TEST_DATE,
    )
...
async def test_delete_new_device_key(some_tokens_repo):
    repo = some_tokens_repo

    assert await repo.delete_new_device_key() is None
    # we cannot say if there is ot not without creating it?


async def test_delete_new_device_key_when_empty(empty_repo):
    repo = empty_repo

    assert await repo.delete_new_device_key() is None


async def test_use_invalid_mnemonic_new_device_key(some_tokens_repo):
    repo = some_tokens_repo
...
async def test_use_not_exists_mnemonic_new_device_key(
...
async def test_use_mnemonic_new_device_key(empty_repo):
    repo = empty_repo
    key = await repo.get_new_device_key()
    assert key is not None

    mnemonic_phrase = mnemonic_from_hex(key.key)

    new_token = await repo.use_mnemonic_new_device_key(
        device_name="imnew",
...
async def test_use_mnemonic_expired_new_device_key(
    some_tokens_repo,
):
    repo = some_tokens_repo
    expiration = ten_minutes_into_past()
...
async def test_use_mnemonic_new_device_key_when_empty(empty_repo):
    repo = empty_repo
