...
class MigrateUsersFromJson(Migration):
    ...
    def _get_users_to_migrate(self):
        ...
        logger.info(f"Users in json repo: {[user.username for user in json_repo_users]}")
        logger.info(f"Users in kanidm repo: {[user.username for user in kanidm_repo_users]}")
