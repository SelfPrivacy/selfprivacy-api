...
from selfprivacy_api.services import flake_service_manager
...
class MergeSpModulesFlake(Migration):
    ...
    async def is_migration_needed(self) -> bool:
        return os.path.isdir(flake_service_manager.LEGACY_SP_MODULES_DIR)

    async def migrate(self) -> None:
        current_services = await evaluate_nix_file(
            os.path.join(flake_service_manager.LEGACY_SP_MODULES_DIR, "flake.nix"),
            "f: f.inputs",
        )

        async with FlakeServiceManager(allow_legacy_sp_modules=True) as manager:
            manager.inputs.pop("sp-modules", None)
            for key, value in current_services.items():
                manager.services[key] = value["url"]

        os.rename(
            flake_service_manager.LEGACY_SP_MODULES_DIR,
            LEGACY_SP_MODULES_BACKUP_DIR,
        )
